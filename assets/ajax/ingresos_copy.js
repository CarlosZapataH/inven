var Tbl_Personal, Tbl_Material, idSignature;
...
function load_CamposIngresos(id) {
    ...
    $.get('../controller/MaterialController.php?action=loadCampos_Ingresos', {'idsu': id}, function (response) {
...
$(document).on('click','#AddPersonal',function(){
    "use strict";
    let thisIdServiceUser = $('#IdServicioUsuario').val();
    let divPTabla = $('#divPTabla');
    let divResponse = $('#divPResponse');
    divPTabla.show();
    divResponse.empty();
    sga.blockUI.loading_body();
    $.get('../controller/ColaboradorController.php?action=loadCampos_addPersonal', {'id':thisIdServiceUser}, function (response) {
        divResponse.html(response);
    }).always(function () {
        sga.blockUI.unblock_body();
        divPTabla.hide();
...
$(document).on('submit','#guardarDatosIniciales_Personal', function(e) {
    "use strict";
    e.preventDefault();
    let data = $(this).serialize();
    Swal.fire({
        html: 'Se va iniciar el proceso de registro del Personal.<br>Desea continuar...!.',
        type: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        confirmButtonText: "Aceptar",
        ...
        if (result.value) {
            sga.blockUI.loading_body();
            $.post('../controller/ColaboradorController.php?action=registrar_datosInitial_JSON', data, function (response) {
                if (parseInt(response.status) === 1) {
                    sga.plugins.toastr('success', 'toast-top-right', response.message, 'Success');
                    //load_INI_SeccionTwo_Personal(response.idcoltk);
                    resetIni_CamposPersonal();
                }
                else if (parseInt(response.status) === 2) {
                    sga.error.show('danger', 'divError', response.message);
...
$(document).on('click','#editPersonal',function(){
    "use strict";
    let thisIdColaborator = $(this).attr('data-id');
    let divPTabla = $('#divPTabla');
    let divResponse = $('#divPResponse');
    divPTabla.show();
    divResponse.hide();
    divResponse.empty();
    sga.blockUI.loading_body();
    $.get('../controller/ColaboradorController.php?action=loadCampos_editPersonal', {'id':thisIdColaborator}, function (response) {
        divResponse.html(response);
    }).always(function () {
        sga.blockUI.unblock_body();
        divPTabla.hide();
...
$(document).on('submit','#guardarDatosPersonal', function(e) {
    "use strict";
    e.preventDefault();
    let data = $(this).serialize();
    Swal.fire({
        html: 'Se va a modificar los datos del registro del Personal.<br>Desea continuar...!.',
        type: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        confirmButtonText: "Aceptar",
        ...
        if (result.value) {
            sga.blockUI.loading_body();
            $.post('../controller/ColaboradorController.php?action=actualizar_datosPersonal_JSON', data, function (response) {
                if (parseInt(response.status) === 1) {
                    sga.plugins.toastr('success', 'toast-top-right', response.message, 'Success');
                    resetIni_CamposPersonal();
                }
                ...
                sga.plugins.toastr('error', 'toast-top-right', 'Se produjo un error al intentar actualizar los datos del Personal, vuelva a intentarlo y si el problema persiste contáctese con el Administrador.', 'Error');
            }).always(function () {
                sga.blockUI.unblock_body();
            });
        }
...
function load_INI_SeccionTwo_Personal(idToken) {
    "use strict";
    let divResponse = $('#divPResponse');
    divResponse.show();
    sga.blockUI.loading_body();
    $.get('../controller/ColaboradorController.php?action=load_Seccion2_Personal', {'id':idToken}, function (response) {
        divResponse.empty();
        divResponse.html(response);
        sga.plugins.inputFile('#file_signature','img');
    }).always(function () {
        sga.blockUI.unblock_body();
        sga.funcion.pageTop();
    });
...
$(document).on('submit','#saveSignature_Personal',function(e){
    "use strict";
    e.preventDefault();
    let remoteServer = $('#remoteserver').val();
    let idcolTk = $('#idcol_tk').val();
    let formdata = new FormData($(this)[0]);
    formdata.append('remoteserver',remoteServer);
    formdata.append('idcoltk',idcolTk);
    $.ajax({
        url: '../controller/ColaboradorController.php?action=registrar_datosFirmaElectronica_JSON',
        type: "POST",
        data: formdata,
        dataType: "json",
        cache: false,
        ...
        processData:false, //Evitamos que JQuery procese los datos, daría error
        beforeSend: function(){
            sga.plugins.inputFile_disable('#file_signature');
            sga.blockUI.loading_body();
        },
        success: function (response) {
            if(parseInt(response.status)===0){
                sga.plugins.inputFile_clear('#file_signature');
                sga.plugins.inputFile_enable('#file_signature');
                sga.plugins.toastr('error', 'toast-top-right', response.message, 'Error');
            }
            else if(parseInt(response.status)===1){
                sga.plugins.toastr('success', 'toast-top-right', response.message, 'Success');
                resetIni_CamposPersonal();
                //load_INI_SeccionThree_Personal(idcolTk);
            }
        }
    }).fail(function (e) {
        sga.plugins.toastr('error', 'toast-top-right', 'Se produjo un error al intentar cargar la Firma digital, vuelva a intentarlo y si el problema persiste contáctese con el Administrador.', 'Error');
        sga.plugins.inputFile_clear('#file_data');
        sga.plugins.inputFile_enable('#file_data');
    }).always(function () {
        sga.blockUI.unblock_body();
...
function load_INI_SeccionThree_Personal(idToken) {
    "use strict";
    let divResponse = $('#divPResponse');
    divResponse.show();
    sga.blockUI.loading_body();
    $.get('../controller/ColaboradorController.php?action=load_Seccion3_Personal', {'id':idToken}, function (response) {
        divResponse.empty();
        divResponse.html(response);
        //
        //let redrawSignature =  $('#redrawSignature').signature({thickness: 4});
        //redrawSignature.signature('draw', dataImg);
    }).always(function () {
        sga.blockUI.unblock_body();
        sga.funcion.pageTop();
    });
...
function resetIni_CamposPersonal() {
    "use strict";
    let divPTabla = $('#divPTabla');
    let divResponse = $('#divPResponse');
    divPTabla.show();
...
$(document).on('click','#ImportPersonal',function(){
    "use strict";
    let IdServicioUsuario = $('#IdServicioUsuario').val();
    let divTabla = $('#divPTabla');
    let divResponse = $('#divPResponse');
    divTabla.show();
    divResponse.hide();
    sga.blockUI.loading_body();
    $.get('../controller/ColaboradorController.php?action=loadCampos_importColaborador', {'idsu':IdServicioUsuario}, function (response) {
        divResponse.html(response);
    }).always(function () {
        sga.blockUI.unblock_body();
        divTabla.hide();
...
$(document).on('submit','#fmrLoad_DataPersonal',function(e){
    ...
    let idServicioUsuario = $('#IdServicioUsuario').val();
    let formdata = new FormData($(this)[0]);
    formdata.append('idServicioUsuario',idServicioUsuario);
    $.ajax({
