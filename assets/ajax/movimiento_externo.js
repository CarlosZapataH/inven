...
function load_Almacenes_xServicioUsuario(id) {
  ...
  if (id !== "") {
    $.get(
      "../controller/AlmacenController.php?action=loadSelect_Almacen_ServicioUsuario_JSON",
      { idsu: id },
      function (lista) {
        $selectID.empty();
...
$(document).on("click", "#btnAction_Search", function () {
  ...
  if (IdAlmacen.val() !== "" && transac.val() !== null) {
    ...
    sga.error.show(
      "danger",
      "mensajes_actions_rpte",
      "Debe completar los campos Principales para realizar la búsqueda: <b>Almacén</b> y <b>Transacción</b>."
    );
    window.setTimeout(function () {
      $("#mensajes_actions_rpte").html("");
    }, 6000);
  }
});

$(document).on("click", "#dataExportPDF_Guia", function () {
  "use strict";
  let idTransfer = $(this).attr("data-id");
  let optionTransfer = $(this).attr("data-opt");
...
function generarExport_Guia(idTransfer, optionTransfer) {
  ...
  modalDefault.empty();
  let page =
    "../app/Export-Guia-PDF-Item.php?idTransfer=" +
    idTransfer +
    "&option=" +
    optionTransfer;
  $.ajax({
...
function loadTbl_Inventario_Transacciones(datos) {
  ...
  Tbl_Reporte = $("#Tbl_Reporte").DataTable({
    ...
    initComplete: function (settings, json) {
      sga.blockUI.unblock_body();
      let dtBbuttons = $(
        "#Tbl_Reporte_wrapper > div.datatable-header > div.dt-buttons"
      );
      dtBbuttons.empty();
      if (
        parseInt(optionImportExport) === 1 &&
        parseInt(json.data.length) > 0
      ) {
        let page =
          "../../app/movimientos-Export-Externo.php?almacen=" +
          datos.almacen +
          "&transac=" +
          datos.transac +
          "&fecha=" +
          datos.fecha;
        let btnExport =
          '<a class="btn btn-success btn-hover-transform cursor-pointer mr-10 btn-sm" href="' +
          page +
          '"><i class="fa fa-file-excel-o mr-10 fz-18"></i>Exportar</a>';
...
$(document).ready(function () {
  $("#openCosolidate").on("click", function () {
    ...
    if (
      ...
      alertMessage =
        "Debes seleccionar al menos un ítem de la lista para continuar";
    } else if (!validStore(seleccionados, 4, originStore)) {
      alertMessage =
        "Antes de continuar, asegúrate de que todos los almacenes seleccionados tengan el mismo origen";
    } else if (!validStore(seleccionados, 5, destinationStore)) {
      alertMessage =
        "Antes de continuar, asegúrate de que todos los almacenes seleccionados tengan el mismo destino";
    } else if (!validateStatus(seleccionados)) {
      alertMessage =
        'Por favor, selecciona solo los movimientos con estado "Disponible" antes de continuar';
    }
    ...
    if (!!alertMessage) {
      swal.fire({
        ...
        text:
          alertMessage ||
          "Lo siento, parece que has seleccionado incorrectamente los elementos de la tabla.",
